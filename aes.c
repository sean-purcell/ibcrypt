...
/* Speedup table for rijndael mix-columns */
uint8_t tm02[256] = {
...
int create_key_AES(const uint8_t* const source, const int bits, AES_KEY* const key) {
	...
	const int n = bits / 8; /* bytes of key */ 
...
void encrypt_block_AES(const uint8_t* const in, uint8_t* const out, const AES_KEY* const key) {
	...
	for(int i = 1; i < rounds; i++) { /* iterate AES rounds */
		sub_bytes(out);
		shift_rows(out);
		mix_columns(out);
...
void decrypt_block_AES(const uint8_t* const in, uint8_t* const out, const AES_KEY* const key) {
	...
	for(int i = rounds - 1; i > 0; i--) { /* iterate AES inverse rounds */
		shift_rows_inv(out);
		sub_bytes_inv(out);
		add_round_key(out, key, i);
		mix_columns_inv(out);
