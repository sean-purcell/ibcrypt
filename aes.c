...
void add_round_key(uint8_t* const buf, const AES_KEY* const key, const int round) {
	for(int i = 0; i < 16; i++) {
		buf[i] ^= key->rd_key[round * 16 + i];
	}
}

void sub_bytes(uint8_t* const buf) {
	for(int i = 0; i < 16; i++) {
		buf[i] = sbox[buf[i]];
	}
}

void sub_bytes_inv(uint8_t* const buf) {
	for(int i = 0; i < 16; i++) {
		buf[i] = sbox_inv[buf[i]];
	}
}

void shift_rows(uint8_t* const buf) {
	uint8_t f0, f1;
	f0      = buf[ 1];
	buf[ 1] = buf[ 5];
	buf[ 5] = buf[ 9];
...
void shift_rows_inv(uint8_t* const buf) {
	uint8_t f0, f1;
	f0      = buf[ 1];
	buf[ 1] = buf[13];
	buf[13] = buf[ 9];
...
void mix_columns(uint8_t* const buf) {
	for(int i = 0; i < 4; i++) {
...
void mix_columns_inv(uint8_t* const buf) {
	for(int i = 0; i < 4; i++) {
